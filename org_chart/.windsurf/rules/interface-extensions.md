...
- Don't import any Airtable Blocks UI elements like `Box` as these are not supported in Interface
  Extensions
- The entrypoint for an Interface Extension is `frontend/index.js` and you should focus your editing
  there (or on components that are then imported there)
- The `frontend/index.js` file should conclude with an `initializeBlock` call that looks like:
