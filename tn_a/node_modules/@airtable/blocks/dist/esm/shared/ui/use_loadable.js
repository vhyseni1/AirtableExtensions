 ...
 * @hook
 */
export default function useLoadable(models) {
  ...
  const compactModels = useMemo(() => {
    ...
    for (const model of compacted) {
      if (typeof model.loadDataAsync !== 'function') {
        throw spawnError('useLoadable called with %s, which is not a loadable', typeof model === 'object' ? model.toString() : typeof model);
      }
    }
    return compacted;
  }, [constModels]);
  ...
  if (shouldSuspend && !areAllModelsLoaded) {
    const suspensePromise = Promise.all(compactModels.map(model => model.loadDataAsync())).then(() => {
