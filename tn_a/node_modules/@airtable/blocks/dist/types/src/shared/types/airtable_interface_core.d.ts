...
import { type GlobalConfigUpdate, type GlobalConfigData, type GlobalConfigPath, type GlobalConfigPathValidationResult } from './global_config';
...
/** @hidden */
type CellValueValidationResult = {
    isValid: true;
} | {
    isValid: false;
    reason: string;
...
export interface FieldTypeProviderCore {
    ...
    convertCellValueToString(appInterface: AppInterface, cellValue: unknown, fieldData: FieldDataCore): string;
    getCellRendererData(appInterface: AppInterface, cellValue: unknown, fieldData: FieldDataCore, shouldWrap: boolean): {
...
export interface AirtableInterfaceCore<SdkModeT extends SdkMode> {
    ...
    applyMutationAsync(mutation: SdkModeT['MutationT'], opts?: {
        holdForMs?: number;
    }): Promise<void>;
    checkPermissionsForMutation(mutation: SdkModeT['PartialMutationT'], basePermissionData: SdkModeT['BasePermissionDataT']): PermissionCheckResult;
    /**
     * internal utils
     */
...
export {};
//# sourceMappingURL=airtable_interface_core.d.ts.map